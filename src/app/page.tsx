...
export default function LoginPage() {
  ...
  const [selectedPlan, setSelectedPlan] = useState('30'); // الباقة الافتراضية
  const [isLoading, setIsLoading] = useState(false);
  const router = useRouter();

  // تفاصيل الباقات المتاحة
  const plans = [
  ...
  return (
    <div className="min-h-screen bg-[#07090e] text-white flex items-center justify-center p-4 relative overflow-hidden font-sans py-10">
      ...
      <div className="relative z-10 bg-[#101726]/90 backdrop-blur-xl border border-gray-800/80 p-6 md:p-8 rounded-3xl w-full max-w-xl shadow-2xl shadow-black/50 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-gray-400 text-xs">اختر باقة الاشتراك المناسبة لاحتياجات تداولك وسجل دخولك فوراً</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div className="space-y-3">
            <label className="block text-xs font-medium text-gray-300">اختر باقة الاشتراك الشهري:</label>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {plans.map((plan) => (
                <div 
                  key={plan.id}
                  onClick={() => setSelectedPlan(plan.id)}
                  className={`cursor-pointer border rounded-2xl p-4 transition relative flex flex-col justify-between ${
                ...
                >
                  {plan.popular && (
                    <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-gradient-to-r from-emerald-500 to-teal-500 text-[10px] font-bold px-2.5 py-0.5 rounded-full text-white shadow">
                      الأكثر طلباً
                    </span>
                  )}
                  <div>
                    <h3 className="text-xs font-semibold text-gray-300 mb-1">{plan.name}</h3>
                    <p className="text-lg font-bold text-white mb-2">{plan.price}</p>
                  </div>
          ...
          >
            {isLoading ? (
              <>
                ...
                <span>جاري تفعيل الباقة وفتح اللوحة...</span>
              </>
            ) : (
              <span>تأكيد الاشتراك والدخول للوحة التحكم 🚀</span>
            )}
          </button>
        </form>
        ...
        <div className="text-center pt-2 border-t border-gray-800/60">
          <p className="text-[11px] text-gray-500">تمتع بمرونة التوليد الفوري ومراقبة الروابط بأسعار تنافسية</p>
        </div>
